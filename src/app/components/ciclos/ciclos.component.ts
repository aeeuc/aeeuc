...
crearIntervalo() {
  ...
  const vector = new FormArray([])

  const intervalo = new Set()
  ...
  if (inicio <= fin) {
    ...
    // alguna etiqueta
    arreglo.forEach(v => {
      const aux = this.lineChartLabels.findIndex(val => {
        return val === v
      })
...
guardarGrafica() {
  ...
  const values = (<FormArray> this.fomulario_data_clicos.get('fila') ).value
