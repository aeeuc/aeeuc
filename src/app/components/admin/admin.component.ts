...
export class AdminComponent implements OnInit {
  ...
  crearUsuario() {
    ...
    if (this.form_agregar_usuario.valid) {
      ...
      const val = this.form_agregar_usuario.value

      this.usuarioService.getUser(val.username).subscribe((res: any) => {

        if (!res.usuario) {

          if (val.password === val.passconfirm) {

            const user: Usuario = new Usuario('', val.username, val.nombre, val.password)
  ...
  editarUsuario() {
    ...
    if (this.form_agregar_usuario.valid) {
      ...
      const val = this.form_agregar_usuario.value
      const user: Usuario = new Usuario(val.id, val.username, val.nombre, '')
  ...
  cambiarPass() {
    ...
    if (this.form_agregar_usuario.valid) {
      ...
      if (val.password === val.passconfirm) {
        ...
        const user: Usuario = new Usuario(val.id, '', '', val.password)
  ...
  eliminarUsuario() {
    ...
    const user = this.listUser[this.pos]
