  ...
  constructor(
    ...
    activatedRoute.params.subscribe(params => {
      ...
      this.dependenciaService.getDependencia(params['id_dependecia']).subscribe((res: any) => {
        ...
        tipoCargaService.getCargas().subscribe((response: any) => {
          this.tipoCarga = response.data
        ...
        this.institucionService.getInstitucion(this.dependencia_activa.id_institucion).subscribe((response: any) => {
          this.institucion_activa = response.data
        })
      })
    })
...
// ============================================================================================
  ...
  cambio(e: number) {
    ...
    const val = this.censoDeCarga.get('fila').get(`${e}`).value.categoria
  ...
  guardarNewCarga() {
    ...
    if (this.newCarga.valid) {
      ...
      const index = this.tipoCarga.findIndex((e) => {
        return e.categoria === this.newCarga.value.tipo
      })

      // busco el indice del elemento en items
      const j = this.tipoCarga[index].items.findIndex(e => {
      ...
      if (j > -1) {
        ...
        const c: TipoCarga = new TipoCarga(
  ...
  guardarCambios() {
    ...
    let isValid = true
    const filas = (<FormArray>this.censoDeCarga.controls['fila']).controls
    ...
    if (isValid) {
      ...
      const indicadores_LBE = [0, 0, 0, 0, 0, 0, 0]

      dataCenso.forEach(carga => {

        const cargaValue = carga.numero * carga.potencia * carga.fd * carga.horasDia * carga.diaSemana
